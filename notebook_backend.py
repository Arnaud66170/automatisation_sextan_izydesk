...
# Nom du fichier Izydesk
fichier_import = "export_brut_izydesk_rochplaza.xlsx"
data_izydesk = pd.read_excel(fichier_import)
# sauvegarde des datas originales
df_sextan = data_sextan.copy()
df_izydesk = data_izydesk.copy()
# Standardisation des noms de colonne
...
nom_fichier = os.path.basename(fichier_import).lower()
# 1 - Izydesk
...
data_izydesk = data_izydesk[colonnes]
# # Ajout d'informations sur le corner
# data_izydesk["id_corner"] = "004"
# data_izydesk["nom corner"] = "toulouse"
# colonnes = ['id_corner', 'nom corner'] + [col for col in data_izydesk.columns if col not in ['id_corner', 'nom corner']]
# data_izydesk = data_izydesk[colonnes]
...
df_prix_produits_cleaned.head()
produits_non_extraits
df_prix_produits_cleaned.info()
...
# Traitement des produits n'ayant pas matché
...
verif_pepsi[["produit", "produit_match"]]
...
# Aperçu
merged_data[["produit_sextan", "famille"]].head(30)
# modif / attribution des catégories
...
# Contrôle visuel
print(merged_data["categorie"].value_counts())
merged_data[["produit_sextan", "famille", "categorie"]].head(30)
merged_data.info()
# data_izydesk.to_excel("izydesk_auto.xlsx", index=False)
# data_sextan.to_excel("sextan_auto.xlsx", index =False)
# merged_data.to_excel("merged_data_auto.xlsx", index =False)
# data_izydesk.to_excel("exports/izydesk_auto_toulouse.xlsx", index=False)
# # data_sextan.to_excel("exports/sextan_auto.xlsx", index=False)
# merged_data.to_excel("exports/merged_data_auto_toulouse.xlsx", index=False)
# Définir par défaut
id_corner = ""
nom_corner = ""
for keyword, values in corner_mapping.items():
